...
export type SizePresets =
  ...
  | "large" | "long" | "medium" | "tall" | "compactMedium";
...
type Preset = { width: number; height?: number; aspectRatio: number; borderRadius: number };
...
const DynamicIsland = ({ children, id, ...props }: { children: ReactNode; id: string }) => {
  ...
  const width = Math.min(currentSize.width, 691);
  ...
  return (
    <div className="flex items-center justify-center w-full">
      <motion.div
        id={id}
        className="mx-auto items-center justify-center border border-black/10 bg-black text-center text-black dark:border-white/5"
        animate={{
          width: `${width}px`,
          height: height,
...
type DynamicContainerProps = { className?: string; children?: React.ReactNode };
...
const DynamicContainer = ({ className, children }: DynamicContainerProps) => {
  ...
  return (
    <motion.div
      ...
      transition={{ type: "spring", stiffness, damping, duration: size !== previousSize ? 0.5 : 0.8 }}
...
const DynamicDiv = ({ className, children }: DynamicContainerProps) => {
  const { state } = useDynamicIslandSize();
  const { size, previousSize } = state;
  const willChange = useWillChange();
...
type MotionChildProps = { className: string; children: React.ReactNode };

const DynamicTitle = ({ className, children }: MotionChildProps) => {
  const { state } = useDynamicIslandSize();
  const { size, previousSize } = state;
  const willChange = useWillChange();
...
const DynamicDescription = ({ className, children }: MotionChildProps) => {
  const { state } = useDynamicIslandSize();
  const { size, previousSize } = state;
  const willChange = useWillChange();
