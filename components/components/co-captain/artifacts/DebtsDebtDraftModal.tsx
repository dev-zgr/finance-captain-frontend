...
import { DEBT_CATEGORIES_WITH_LABELS, MAX_DEBT_DESCRIPTION_LENGTH } from "@/lib/debts-account/constants"

type PendingAction = "accept" | "reject" | null

export type DebtsDebtDraftFormValues = {
  date: string
  amount: string
  category: string
  description: string
...
export type DebtsDebtDraftFieldErrors = Partial<Record<keyof DebtsDebtDraftFormValues, string>>

type Props = {
  open: boolean
  onOpenChange: (open: boolean) => void
  values: DebtsDebtDraftFormValues
  fieldErrors: DebtsDebtDraftFieldErrors
  globalError: string
  pendingAction: PendingAction
  disableAccept: boolean
  onChange: (key: keyof DebtsDebtDraftFormValues, value: string) => void
  onAccept: () => void
  onReject: () => void
}

export function DebtsDebtDraftModal({
  open,
  onOpenChange,
  values,
  fieldErrors,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Review draft new debt</DialogTitle>
          <DialogDescription>
        ...
        <FieldGroup className="gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field data-invalid={Boolean(fieldErrors.amount)}>
              <FieldLabel htmlFor="draft-debt-amount">Amount</FieldLabel>
              <FieldContent>
                <InputGroup>
                  <InputGroupAddon>
            ...
            <Field data-invalid={Boolean(fieldErrors.date)}>
              <FieldLabel htmlFor="draft-debt-date">Date</FieldLabel>
              <FieldContent>
                <DatePicker
                  value={values.date}
          ...
          <Field data-invalid={Boolean(fieldErrors.category)}>
            <FieldLabel htmlFor="draft-debt-category">Category</FieldLabel>
            <FieldContent>
              <ComboboxSelect
          ...
          <Field data-invalid={Boolean(fieldErrors.description)}>
            <FieldLabel htmlFor="draft-debt-description">Description</FieldLabel>
            <FieldContent>
              <Input
                id="draft-debt-description"
                type="text"
