...
import { mapBackendPayDebtFieldErrors, toBackendDate, validatePayDebtForm } from "@/lib/debts-account/validation"
import { acceptDraft, rejectDraft } from "@/lib/co-captain/api"
import type {
  ApiErrorEnvelope,
  Artifact,
  ArtifactRendererProps,
  DebtsPaymentDraftPayload,
} from "@/lib/co-captain/types"
import {
  DebtsPaymentDraftModal,
  type DebtsPaymentDraftFieldErrors,
  type DebtsPaymentDraftFormValues,
} from "./DebtsPaymentDraftModal"
...
function parseDebtsPaymentDraftPayload(payload: unknown): DebtsPaymentDraftPayload {
  const raw = (payload ?? {}) as Partial<DebtsPaymentDraftPayload>
...
function toFormValues(payload: DebtsPaymentDraftPayload): DebtsPaymentDraftFormValues {
  return {
...
function requiredFieldErrors(values: DebtsPaymentDraftFormValues): DebtsPaymentDraftFieldErrors {
  const errors: DebtsPaymentDraftFieldErrors = {}
...
function isSameFormValues(left: DebtsPaymentDraftFormValues, right: DebtsPaymentDraftFormValues): boolean {
  return left.date === right.date
    && left.amount === right.amount
    && left.description === right.description
}

export function DebtsPaymentDraftArtifact({ artifact, token, onUpdate }: ArtifactRendererProps<unknown>) {
  const [localArtifact, setLocalArtifact] = useState(artifact)
  const [open, setOpen] = useState(false)
  const [pendingAction, setPendingAction] = useState<PendingAction>(null)
  const [formValues, setFormValues] = useState<DebtsPaymentDraftFormValues>(() =>
    toFormValues(parseDebtsPaymentDraftPayload(artifact.payload)),
  )
  const [fieldErrors, setFieldErrors] = useState<DebtsPaymentDraftFieldErrors>({})
  const [cardError, setCardError] = useState("")
  const [modalError, setModalError] = useState("")
  const [hasLocalEdits, setHasLocalEdits] = useState(false)

  const sourcePayload = useMemo(
    () => parseDebtsPaymentDraftPayload(artifact.payload),
    [artifact.payload],
  )
  ...
  const payload = useMemo(() => parseDebtsPaymentDraftPayload(localArtifact.payload), [localArtifact.payload])
  ...
  const liveValidationErrors = useMemo(
    () => ({
      ...validatePayDebtForm(formValues),
  ...
  function applyEditsToDraft(nextValues: DebtsPaymentDraftFormValues) {
    const nextAmount = Number(nextValues.amount)
    const nextPayload: DebtsPaymentDraftPayload = {
  ...
  function handleFormChange(key: keyof DebtsPaymentDraftFormValues, value: string) {
  ...
  function buildSubmitPayload(): DebtsPaymentDraftPayload {
    return {
  ...
  async function handleQuickAccept() {
    ...
    try {
      ...
      if (response.status === 422) {
        ...
        setFieldErrors(mapBackendPayDebtFieldErrors(data.fieldErrors))
                ...
                <div className="flex items-start gap-1.5">
                  ...
                  <div className="min-w-0 space-y-0.5">
                    <p className="truncate text-[11px] font-semibold">
                      Draft debt payment:{" "}
                      <span className="text-emerald-600 dark:text-emerald-500">{amountText}</span>
                    </p>
                    <p className="truncate text-[10px] text-muted-foreground">
                      {dateText} · &quot;{descriptionText}&quot;
                    </p>
                  </div>
                </div>
              </div>
          ...
          {isAccepted ? (
            <div className="pr-24">
              <div className="flex items-start gap-1.5">
                ...
                <div className="min-w-0 space-y-0.5">
                  <p className="truncate text-[11px] font-semibold">Logged: {amountText} debt payment</p>
                  <p className="truncate text-[10px] text-muted-foreground">{dateText}</p>
