...
import { DEBT_CATEGORIES_WITH_LABELS } from "@/lib/debts-account/constants"
import { cn } from "@/lib/utils"
import type { DebtsTransactionListPayload } from "@/lib/co-captain/types"
...
const buildQueryParams = (filters: {
  ...
  return query ? `/debt-account/transactions?${query}` : "/debt-account/transactions"
}
...
export interface DebtsTransactionListModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  payload: Partial<DebtsTransactionListPayload>
}

export function DebtsTransactionListModal({
  open,
  onOpenChange,
  payload,
}: DebtsTransactionListModalProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-5xl h-[85vh] overflow-hidden flex flex-col gap-0 p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b">
          <DialogTitle>Debt Transactions</DialogTitle>
          {appliedFiltersDescription && (
            <DialogDescription>{appliedFiltersDescription}</DialogDescription>
          )}
          {hasMoreResults && (
            <DialogDescription>
              Showing {displayedCount} of {totalCount} transactions. View all transactions for the complete list.
            </DialogDescription>
          )}
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            <ToggleGroup
              ...
              value={typeFilter}
              onValueChange={(v) => {
                setTypeFilter((v as "" | "DEBT" | "PAYMENT") ?? "")
                setSelectedCategories([])
                setPage(0)
              }}
            >
                ...
                <PopoverContent className="w-72 p-0" align="start">
                  <Command>
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        {DEBT_CATEGORIES_WITH_LABELS.map((opt) => {
                    ...
                    <TableRow key={transaction.id}>
                      ...
                      <TableCell>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => router.push(`/debt-account/transactions/${transaction.id}`)}
                        >
                          Details
