...
import type {
  ...
  InvestmentWithdrawDraftPayload,
} from "@/lib/co-captain/types"
import { formatCurrency } from "@/lib/utils"
import {
  InvestmentWithdrawDraftModal,
  type InvestmentWithdrawDraftFieldErrors,
  type InvestmentWithdrawDraftFormValues,
} from "./InvestmentWithdrawDraftModal"
...
function parsePayload(payload: unknown): InvestmentWithdrawDraftPayload {
  const raw = (payload ?? {}) as Partial<InvestmentWithdrawDraftPayload>
  return {
...
function toFormValues(payload: InvestmentWithdrawDraftPayload): InvestmentWithdrawDraftFormValues {
  return {
...
function mapFieldErrors(fieldErrors?: Record<string, string>): InvestmentWithdrawDraftFieldErrors {
  const nextErrors: InvestmentWithdrawDraftFieldErrors = {}
...
function validationErrorsFor(values: InvestmentWithdrawDraftFormValues): InvestmentWithdrawDraftFieldErrors {
  const errors: InvestmentWithdrawDraftFieldErrors = {}
...
function isSameFormValues(left: InvestmentWithdrawDraftFormValues, right: InvestmentWithdrawDraftFormValues): boolean {
  return left.amount === right.amount && left.description === right.description
}

export function InvestmentWithdrawDraftArtifact({ artifact, token, onUpdate }: ArtifactRendererProps<unknown>) {
  const [localArtifact, setLocalArtifact] = useState(artifact)
  const [open, setOpen] = useState(false)
  const [pendingAction, setPendingAction] = useState<PendingAction>(null)
  const [formValues, setFormValues] = useState<InvestmentWithdrawDraftFormValues>(() =>
    toFormValues(parsePayload(artifact.payload)),
  )
  const [fieldErrors, setFieldErrors] = useState<InvestmentWithdrawDraftFieldErrors>({})
  ...
  function applyEditsToDraft(nextValues: InvestmentWithdrawDraftFormValues) {
    const nextAmount = Number(nextValues.amount)
    const nextPayload: InvestmentWithdrawDraftPayload = {
  ...
  function handleFormChange(key: keyof InvestmentWithdrawDraftFormValues, value: string) {
  ...
  function buildSubmitPayload(): InvestmentWithdrawDraftPayload {
    return {
                ...
                <div className="flex items-start gap-1.5">
                  ...
                  <div className="min-w-0 space-y-0.5">
                    <p className="truncate text-[11px] font-semibold">
                      Draft withdraw: <span className="text-amber-700 dark:text-amber-500">{amountText}</span>
                    </p>
                    <p className="truncate text-[10px] text-muted-foreground">
                      From investment → checking
          ...
          {isAccepted ? (
            <div className="pr-24">
              <div className="flex items-start gap-1.5">
                ...
                <div className="min-w-0 space-y-0.5">
                  <p className="truncate text-[11px] font-semibold">Logged: withdraw of {amountText}</p>
