...
export type InvestmentBuyDraftFormValues = {
  ticker: string
  quantity: string
  description: string
}

export type InvestmentBuyDraftFieldErrors = Partial<Record<keyof InvestmentBuyDraftFormValues, string>>

type Props = {
  open: boolean
  onOpenChange: (open: boolean) => void
  values: InvestmentBuyDraftFormValues
  fieldErrors: InvestmentBuyDraftFieldErrors
  globalError: string
  pendingAction: PendingAction
  disableAccept: boolean
  onChange: (key: keyof InvestmentBuyDraftFormValues, value: string) => void
  onAccept: () => void
  onReject: () => void
  ...
  highlightAvailableCash: boolean
}

const MAX_DESCRIPTION_LENGTH = 256

export function InvestmentBuyDraftModal({
  open,
  onOpenChange,
  values,
  fieldErrors,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>Review draft buy</DialogTitle>
          <DialogDescription>
        ...
        <FieldGroup className="gap-4">
          ...
          <Field data-invalid={Boolean(fieldErrors.ticker)}>
            <FieldLabel htmlFor="draft-investment-buy-ticker">Ticker</FieldLabel>
            <FieldContent>
              <TickerCombobox
                value={values.ticker}
          ...
          <Field data-invalid={Boolean(fieldErrors.quantity)}>
            <FieldLabel htmlFor="draft-investment-buy-quantity">Quantity</FieldLabel>
            <FieldContent>
              <Input
                id="draft-investment-buy-quantity"
                type="text"
                inputMode="decimal"
                placeholder="0.0"
          ...
          <Field data-invalid={Boolean(fieldErrors.description)}>
            <FieldLabel htmlFor="draft-investment-buy-description">Description (optional)</FieldLabel>
            <FieldContent>
              <Input
                id="draft-investment-buy-description"
                type="text"
          ...
          <div className="space-y-1 rounded-md border border-border bg-muted/40 p-3">
            ...
            <p className={`text-xs ${highlightAvailableCash ? "font-medium text-red-700" : "text-muted-foreground"}`}>
              Available cash:
              <span className="ml-2 font-medium text-foreground">
          ...
          {showCashWarning ? (
            <Alert className="border-amber-500/30 bg-amber-500/10 text-amber-800">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                Estimated total exceeds available cash. You can still submit and the server will validate.
              </AlertDescription>
            </Alert>
          ) : null}
        </FieldGroup>
