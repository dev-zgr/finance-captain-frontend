...
import { getInvestmentSummary, getStockDetails } from "@/lib/investment-account/api"
...
import type {
  ...
  InvestmentBuyDraftPayload,
} from "@/lib/co-captain/types"
import {
  InvestmentBuyDraftModal,
  type InvestmentBuyDraftFieldErrors,
  type InvestmentBuyDraftFormValues,
} from "./InvestmentBuyDraftModal"
...
function parsePayload(payload: unknown): InvestmentBuyDraftPayload {
  const raw = (payload ?? {}) as Partial<InvestmentBuyDraftPayload>
  return {
...
function toFormValues(payload: InvestmentBuyDraftPayload): InvestmentBuyDraftFormValues {
  return {
...
function mapFieldErrors(fieldErrors?: Record<string, string>): {
  errors: InvestmentBuyDraftFieldErrors
  hasAmountError: boolean
} {
  const nextErrors: InvestmentBuyDraftFieldErrors = {}
  ...
  return { errors: nextErrors, hasAmountError }
}

function validationErrorsFor(values: InvestmentBuyDraftFormValues): InvestmentBuyDraftFieldErrors {
  const errors: InvestmentBuyDraftFieldErrors = {}
...
function isSameFormValues(left: InvestmentBuyDraftFormValues, right: InvestmentBuyDraftFormValues): boolean {
...
export function InvestmentBuyDraftArtifact({ artifact, token, onUpdate }: ArtifactRendererProps<unknown>) {
  const [localArtifact, setLocalArtifact] = useState(artifact)
  const [open, setOpen] = useState(false)
  const [pendingAction, setPendingAction] = useState<PendingAction>(null)
  const [formValues, setFormValues] = useState<InvestmentBuyDraftFormValues>(() =>
    toFormValues(parsePayload(artifact.payload)),
  )
  const [fieldErrors, setFieldErrors] = useState<InvestmentBuyDraftFieldErrors>({})
  ...
  const [highlightAvailableCash, setHighlightAvailableCash] = useState(false)
  const [cashBalanceFallback, setCashBalanceFallback] = useState<number | null>(null)
  const latestPriceRequestRef = useRef(0)

  const storeCashBalance = useSelector((state: RootState) => state.investmentAccount.summary?.cashBalance ?? null)
  ...
  useEffect(() => {
    ...
    setLiveCompanyName("")
    setLivePriceError("")
    setHighlightAvailableCash(false)
  ...
  const availableCash = storeCashBalance ?? cashBalanceFallback
  ...
  const showCashWarning = useMemo(() => {
    if (availableCash === null || estimatedTotal === null) {
      return false
    }
    return estimatedTotal > availableCash
  }, [availableCash, estimatedTotal])
  ...
  useEffect(() => {
    if (!open || !token || (!isDraft)) {
      return
    }

    const authToken = token

    async function loadBalanceAndPositions() {
      if (storeCashBalance === null) {
        const summaryResponse = await getInvestmentSummary(authToken)
        if (summaryResponse.status === 200 && summaryResponse.data && typeof summaryResponse.data === "object") {
          const wrapped = summaryResponse.data as { content?: unknown; data?: unknown }
          const payloadData = (wrapped.content ?? wrapped.data ?? summaryResponse.data) as { cashBalance?: unknown }
          if (typeof payloadData.cashBalance === "number" && Number.isFinite(payloadData.cashBalance)) {
            setCashBalanceFallback(payloadData.cashBalance)
          }
        }
      }
    ...
    void loadBalanceAndPositions()
  }, [isDraft, open, storeCashBalance, token])
  ...
  function applyEditsToDraft(nextValues: InvestmentBuyDraftFormValues) {
    ...
    const nextPayload: InvestmentBuyDraftPayload = {
      ticker,
      quantity,
  ...
  function handleFormChange(key: keyof InvestmentBuyDraftFormValues, value: string) {
  ...
  async function handleQuickAccept() {
    ...
    try {
      ...
      if (response.status === 422) {
        ...
        const mapped = mapFieldErrors(data.fieldErrors)
        setFieldErrors(mapped.errors)
        setHighlightAvailableCash(mapped.hasAmountError)
  ...
  return (
    <>
                ...
                <div className="flex items-start gap-1.5">
                  ...
                  <div className="min-w-0 space-y-0.5">
                    <p className="truncate text-[11px] font-semibold">
                      Draft buy: {formatQuantity(payload.quantity)} <span className="font-mono">{payload.ticker || "—"}</span>
                    </p>
          ...
          {isAccepted ? (
            <div className="pr-24">
              <div className="flex items-start gap-1.5">
                ...
                <div className="min-w-0 space-y-0.5">
                  <p className="truncate text-[11px] font-semibold">
                    Logged: bought {formatQuantity(payload.quantity)} <span className="font-mono">{payload.ticker || "—"}</span>
                  </p>
      ...
      <InvestmentBuyDraftModal
        ...
        highlightAvailableCash={highlightAvailableCash}
      />
    </>
  )
}
