...
const STRETCH_INNER_CARD = "[&>*]:h-full";
...
export function DashboardSummarySlider({ token, refreshKey }: Props) {
  ...
  return (
    <Tabs
      ...
      className="col-span-5 flex h-full flex-col gap-3 max-lg:col-span-12"
    >
      ...
      <TabsContent
        value="checking"
        className={`flex-1 ${STRETCH_INNER_CARD} ${TAB_CONTENT_ANIMATION}`}
      >
        <AccountSummaryCard token={token} refreshKey={refreshKey} />
      </TabsContent>
      <TabsContent
        value="debts"
        className={`flex-1 ${STRETCH_INNER_CARD} ${TAB_CONTENT_ANIMATION}`}
      >
        <DebtsAccountSummaryCard token={token} refreshKey={refreshKey} />
      </TabsContent>
      <TabsContent
        value="investments"
        className={`flex-1 ${STRETCH_INNER_CARD} ${TAB_CONTENT_ANIMATION}`}
      >
        <InvestmentSummaryCard token={token} refreshKey={refreshKey} />
      </TabsContent>
    </Tabs>
  );
}
